...
=== Guest Node RWO Volume Limits

KubeVirt enforces an API schema limit of **256 total disks/volumes per virtual machine instance (VMI)**. Because guest Kubernetes nodes are provisioned as virtual machines, a single guest node can only attach a finite number of ReadWriteOnce (RWO) volumes as block disks. 

* This limit **includes** non-CSI disks such as the root disk and cloud-init disk. Consequently, the maximum number of RWO CSI volumes a guest node can host is 256 minus the VM's existing disks.
* ReadWriteMany (RWX) volumes do **not** consume these disk slots because they are mounted through NFS.
* If too many RWO PVC-dependent workloads are scheduled on a single guest node, volume attachment will fail with the error: `VM Schema Invalid: Max Disk Limit (256) Exceeded on Guest Cluster`. 
* **Recommendation:** To prevent this issue, distribute RWO PVC-dependent workloads across multiple guest worker nodes.
...
* **CPU Cores:** 1 dedicated CPU core assigned to the V2 instance-manager pod. This is required to handle the busy-polling `spdk_tgt` process, which consumes 100% of a dedicated core to efficiently process I/O requests.

=== Performance, Scalability, and Node Sizing

Because cluster specifications vary widely across environments (e.g., bare-metal vs. cloud instances), storage performance and practical scalability—such as maximum IOPS and maximum volumes per cluster—also vary by environment. 

Rather than enforcing strict dynamic limits, the {longhorn-product-name} project publishes benchmark reports for representative setups. For detailed reference results, node sizing guides, and performance benchmarks, refer to the official https://github.com/longhorn/longhorn/tree/v1.11.2/scalability/reference-setup-performance-scalability-and-sizing-guidelines[{longhorn-product-name} Setup, Performance, Scalability, and Sizing Guidelines].

[NOTE]
====
The link above targets {longhorn-product-name} v1.11.2, which is the version embedded in {harvester-product-name} v1.8.1. If you are running a different version, select the corresponding branch in the {longhorn-product-name} GitHub repository.
====
