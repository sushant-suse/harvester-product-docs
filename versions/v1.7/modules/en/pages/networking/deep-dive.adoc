...
. Create a xref:networking/cluster-network.adoc#_cluster_network[cluster network] and a corresponding xref:networking/cluster-network.adoc#_network_configuration[network configuration]. Only nodes specified in the network configuration set up the associated network devices.

. Create a xref:networking/vm-network.adoc#_create_a_vm_network[VM network] with a specified VLAN ID.

=== Example scenario

This example demonstrates how {harvester-product-name} handles node configuration and virtual machine placement when connecting to a custom cluster network.
...
In response, {harvester-product-name} automatically handles the following:

* *Node labeling*: The {harvester-product-name} controller labels the target Kubernetes `node` objects.
+
[,yaml]
----
...
* *Affinity rules*: The {harvester-product-name} webhook updates the `virtualmachine` object to inject a node affinity rule.
+
[,yaml]
----
  spec:
...
[IMPORTANT]
====
When a virtual machine connects to multiple VM networks backed by different custom cluster networks, {harvester-product-name} applies multiple node affinity rules. These rules collectively determine which nodes are eligible for scheduling and live migration.

Conversely, no node affinity rules are applied when a virtual machine connects exclusively to VM networks backed by xref:networking/cluster-network.adoc#_built_in_cluster_network[`mgmt`] (the built-in cluster network). Because `mgmt` covers all nodes by default, all cluster nodes are eligible for scheduling and live migration.
====
