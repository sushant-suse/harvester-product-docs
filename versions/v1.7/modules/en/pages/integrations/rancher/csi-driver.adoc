...
* {harvester-product-name} v1.4 or later is installed on the host cluster.

* A xref:networking/storage-network.adoc[storage network] is configured on the {harvester-product-name} cluster.
+
...
For more information, see the https://github.com/harvester/harvester/blob/v1.7/deploy/charts/harvester/templates/rbac.yaml[RBAC configuration] in the GitHub repository.