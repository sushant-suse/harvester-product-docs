...
To ensure your changes persist across upgrades, modify the {harvester-product-name} `ManagedChart` resource directly 
rather than the `KubeVirt` resource. Changes to `ManagedChart` are automatically applied to `KubeVirt`.
