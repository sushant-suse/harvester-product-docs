...
Whenever possible, run additional software inside a container. This approach requires no changes to {harvester-product-name}’s immutable operating system image and allows the software to be managed independently of {harvester-product-name}'s standard installation and upgrade process.

== General diagnostic, monitoring, and alerting tools

{harvester-product-name} provides add-ons for logging, monitoring, and alerting using Prometheus, Grafana, and Alertmanager. For setup instructions, see xref:observability/logging.adoc[Logging] and xref:observability/monitoring.adoc[Monitoring].
...
* Interactive utilities: Create a https://harvesterhci.io/kb/package_your_own_toolbox_image/[toolbox container] containing the required utilities. Building from the https://registry.suse.com/repositories/bci-bci-base-16-0[SUSE Linux BCI 16.0 Base Container Image] provides access to the free `SLE_BCI` repository, which includes up-to-date packages from SUSE Linux Enterprise Server.

== Building kernel modules

Starting with {harvester-product-name} v1.9.0, you can use the https://hub.docker.com/r/rancher/harvester-kernel-module-devel/tags[`rancher/harvester-kernel-module-devel`] container image to build kernel modules for {harvester-product-name}'s specific Linux kernel version. This image includes the complete kernel source, GCC, and essential build tools. It also provides access to the free `SLE_BCI` software repository if your build process requires additional dependencies.

To determine the correct image tag, check the `osImage` field in `/etc/harvester-release.yaml` on a {harvester-product-name} host. The following example shows the image tag for {harvester-product-name} v1.9.0-rc6. To build kernel modules for this version, use the matching image (https://hub.docker.com/layers/rancher/harvester-kernel-module-devel/v1.9-20260819[`rancher/harvester-kernel-module-devel:v1.9-20260819`]).
...
To build out-of-tree kernel modules, configure a DaemonSet that compiles the required modules in an `initContainer` and loads them using `modprobe`. The following example demonstrates this concept using DRBD source code from https://linbit.com[LINBIT]:
...
When upgrading {harvester-product-name}, update the `builder` container image tag to match the new operating system image.

=== Building kernel modules for system boot

The following example uses DRBD source code from https://linbit.com[LINBIT]:

. Build the kernel module inside an interactive container using the image tag that matches your {harvester-product-name} version.
+
[,shell]
----
...
. In a separate terminal on your host, copy the compiled module binary out of the running container.
+
[,shell]
----
...
. Copy the kernel module binary to `/var/lib/third-party/` on each {harvester-product-name} node.

. To load the module at boot time, create a CloudInit custom resource similar to the following:
+
[,yaml]
----
...
[NOTE]
====
When upgrading {harvester-product-name}, you must rebuild the module against the new kernel version and copy it to each host again.
====

=== Building and loading in-tree kernel modules

Because the complete kernel source is included in https://hub.docker.com/r/rancher/harvester-kernel-module-devel/tags[`rancher/harvester-kernel-module-devel`], you can build and load any in-tree kernel module using either of the previously outlined methods.
...
. Generate a signing key and certificate with the following command:
+
[,shell]
----
...
[IMPORTANT]
====
Keep the private key (`signing_key.pem`) secure. Do not store unencrypted private keys on general-purpose {harvester-product-name} hosts.
====

. Sign each kernel module by running the following command inside the `rancher/harvester-kernel-module-devel` image.
+
[,shell]
----
...
. Load the public certificate (`signing_key.x509`) onto every host that requires the signed modules.
+
[,shell]
----
...
. Reboot the host and access the console.

. When prompted during startup, select *Perform MOK Management* and follow the prompts to enroll the key.

[NOTE]
====
If you are building modules using a DaemonSet and the `rancher/harvester-kernel-module-devel` image, consider storing signing keys as Kubernetes secrets, similar to the approach used by the https://piraeus.io/docs/v2.11.0/how-to/secure-boot/[Piraeus Datastore project]. Note that manual MOK enrollment on each host is required by Secure Boot architecture.
====

== Installing packages during installation and upgrades

If necessary, you can install software packages directly into {harvester-product-name}'s operating system image. However, this can only occur during initial system installation or system upgrades, not on an ad-hoc basis.

This capability is provided by the {elemental-product-name}'s https://rancher.github.io/elemental-toolkit/docs/customizing/runtime_persistent_changes/[`after-install-chroot` and `after-upgrade chroot` stages]. You will typically define identical command sets in both stages.
...
To install additional software during upgrades, add specific commands to the `after-upgrade-chroot` stage of a xref:installation-setup/config/cloudinitcrd.adoc[CloudInit CR].

As with `after-install-chroot`, the required commands depend on the specific software you are installing. Additionally, you must consider the target operating system version. {harvester-product-name} v1.8.x and v1.9.x are based on SUSE Linux Micro 6.2, which shares the same codebase as SUSE Linux Enterprise (SLE) 16.0. Because future {harvester-product-name} releases will update the underlying SLE base, verify operating system compatibility before upgrading.
